...
function setSession(res, token) {
  // Host-only cookie: works on the actual GoonFx domain without creating a domain mismatch.
  res.setHeader('Set-Cookie', `gx_session=${encodeURIComponent(seal(token))}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=3600`);
}
function clearSession(res) { res.setHeader('Set-Cookie','gx_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0'); }
function getToken(req) { const c=parseCookies(req); return c.gx_session ? unseal(c.gx_session) : null; }
function cors(res) { res.setHeader('Access-Control-Allow-Origin','https://goonfx.com'); res.setHeader('Access-Control-Allow-Credentials','true'); res.setHeader('Access-Control-Allow-Headers','Content-Type'); res.setHeader('Access-Control-Allow-Methods','GET,POST,OPTIONS'); }
...
async function publicWs(request) {
  return await new Promise((resolve,reject)=>{
    ...
    const timer=setTimeout(()=>{try{ws.close()}catch{};reject(new Error('Deriv market data timeout'))},10000);
    ws.onopen=()=>ws.send(JSON.stringify(request));
    ws.onmessage=e=>{clearTimeout(timer);try{const d=JSON.parse(e.data);ws.close();if(d.error)reject(new Error(d.error.message));else resolve(d)}catch(err){reject(err)}};
    ws.onerror=()=>{clearTimeout(timer);reject(new Error('Deriv market data connection failed'))};
  });
}
