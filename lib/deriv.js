...
function setSession(res, token) {
  // The frontend and API are separate origins during the transition, so the
  // authenticated browser session must be permitted on cross-origin fetches.
  res.setHeader('Set-Cookie', `gx_session=${encodeURIComponent(seal(token))}; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=3600`);
}
